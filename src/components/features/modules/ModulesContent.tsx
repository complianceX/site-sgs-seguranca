...
import { ArrowRight, FileText, Users, Activity, ShieldCheck, ClipboardList, AlertTriangle, Search, GraduationCap, Stethoscope, HardHat, Building2, BarChart2, CheckCircle2 } from "lucide-react";
...
const groups = [
  {
    title: "Documentos de SST", description: "Capacidades da plataforma para criação e gestão de documentos críticos.",
    modules: [
  ...
  {
    title: "Pessoas e Conformidade", description: "Gestão do capital humano e requisitos legais de saúde e segurança.",
    modules: [
  ...
  {
    title: "Gestão Operacional", description: "Estrutura e suporte para as atividades do dia a dia.",
    modules: [
  ...
  {
    title: "Inteligência e Governança", description: "Visibilidade, dados e integridade documental.",
    modules: [
  ...
  return (
    <div className="py-24 lg:py-40 bg-white">
      <div className="container">
        <div className="max-w-4xl mb-24">
          <FadeIn direction="none" stagger>
            <div className="inline-flex items-center gap-2.5 px-4 py-2 bg-primary/5 text-primary rounded-full text-[10px] font-black mb-10 tracking-[0.2em] uppercase">
              <HardHat className="w-4 h-4" /> Ecossistema Completo
            </div>
            <MotionText as="h1" className="text-5xl md:text-7xl font-black text-sgs-navy mb-10 tracking-tighter leading-[1.05] text-balance">Cobertura completa da gestão de SST</MotionText>
            <p className="text-xl md:text-2xl text-slate-500 leading-relaxed font-medium text-pretty max-w-3xl">O SGS é composto por módulos integrados que cobrem todas as necessidades de gestão de SST, desde a operação no campo até a governança executiva.</p>
          </FadeIn>
        </div>
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                {group.modules.map((mod, mIdx) => (
                  <MotionCard key={mIdx} delay={mIdx * 0.05}>
                    ...
                    <h3 className="text-xl font-black mb-4 text-sgs-navy tracking-tight group-hover:text-primary transition-colors">{mod.name}</h3>
        ...
        <FadeIn direction="up">
          <div className="mt-32 p-12 lg:p-24 text-center relative overflow-hidden rounded-[3rem] bg-slate-50">
            <div className="relative z-10 max-w-3xl mx-auto">
              ...
              <p className="text-slate-500 mb-12 text-xl font-medium leading-relaxed">Agende uma conversa com nossos especialistas para ver como cada módulo pode ser aplicado à realidade da sua empresa.</p>
              <div className="flex flex-wrap justify-center gap-6">
                <Link href="/contato"><MotionButton size="lg" className="gap-3">Solicitar Demonstração <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" /></MotionButton></Link>
                <Link href="/"><MotionButton variant="outline" size="lg">Voltar ao Início</MotionButton></Link>
              </div>
            </div>
          </div>
        </FadeIn>
